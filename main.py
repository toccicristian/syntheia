...
def datalist_bot():
    ...
    datos = {"botname": botname, "trainer": trainer}
...
def selector(comando, chatbot, trainer):
    ...
    if comando.lower().startswith("/entrenar ") and isfile(normpath(expanduser(comando.lower().lstrip("/entrenar ")))):
        ...
        trainer.train(normpath(expanduser(comando.lower().lstrip("/entrenar "))))
        print("***Entrenamiento terminado!")
        return None
    ...
    if comando.lower() == "/exit":
        ...
        response = chatbot.get_response(comando)
...
while True:
    comando = input(f"{apodo}> ")
    selector(comando, chatbot, trainer)
