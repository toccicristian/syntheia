...
class CustomLogicAdapter(LogicAdapter):
    ...
    def process(self, statement, additional_response_selection_parameters):
        # Procesar la declaración y devolver una respuesta
...
def selector(comando, datos):
    ...
    if comando.lower().startswith("/entrenar ") and isfile(normpath(expanduser(comando.lower().lstrip("/entrenar ")))):
        ...
        datos["trainer"].train(normpath(expanduser(comando.lower().listrip("/entrenar "))))
...
datos["chatbot"] = ChatBot(
    ...
    logic_adapters=[
        ...
        {
            'import_path': 'path.to.CustomLogicAdapter'
        }
    ]
)
